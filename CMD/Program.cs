    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                GET_CTR_REORT_BY_TRANSACTION_CODE("");
        ...
        public static void GET_CTR_REORT_BY_TRANSACTION_CODE(string transCode)
        {

            #region Call Helper Class
            DataSet Headerds = new DataSet();
            DataSet ds = new DataSet();
            _Connection Connections = new _Connection();
            ...
            try
            {
                ...
                EceckD.Trim();
                EceckT.Trim();
                string oneRecord = EE.FirstOrDefault().Field<string>("CODE_FROM_TRANS".ToUpper());
                EceckD = "CTR_" + EceckD + ".xml";
                if (oneRecord == reportType.BANK_DRAFT || oneRecord == reportType.BANK_DRAFT_CREDIT)
                {
                    path = @"C:\SASAMLREPORT\BANK_DRAFT\" + EceckD;
                }
                else if (oneRecord == reportType.CLEARING_CHEQUE || oneRecord == reportType.CLEARING_CHEQUE_CREDIT)
                {
                    path = @"C:\SASAMLREPORT\CLEARING_CHEQUE\" + EceckD;
                }
                else if (oneRecord == reportType.LIQUIDATION)
                {
                    path = @"C:\SASAMLREPORT\LIQUIDATION\" + EceckD;
                }
                #endregion
        ...
        {
            public string CASH_DEPOSIT = "CHDP";
            public string ACCOUNT_TRANSFER = "CHWL";
            public string NIBSS_TRANSFER_FROM_ACCOUNT = "KLNR";
